...
class GameController implements GameInterface {
  ...
  init(): void {
    ...
    EventBus.on('onEnd', msg => {
      ...
      setTimeout(() => {
        // 结束弹框
        const isReStart = confirm(msg + '游戏结束。\n\n是否重新开始？')
        if (isReStart) {
          // 点击确定，继续游戏
  ...
  run(): void {
    ...
    !(this._isEnd || this._isPause) &&
      setTimeout(
        this.run.bind(this),
        this._speed - (this._scorePanel.level - 1) * Math.floor(this._speed / this._scorePanel.maxLevel)
      )
  }
  ...
  cantTurnBack(event: KeyboardEvent): void {
    ...
    // 之前按键队列的最后一个元素
    const last: string = this._direction[this._direction.length - 1]
    if (
